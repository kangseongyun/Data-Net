...
print(list(df_merge_result.columns))


################################### 연도별 구분  ########################################################################

# def df_year(A):
#     df_merge_result_2018=df_merge_result[df_merge_result["year_use"]==A]
#     # print(df_merge_result_2018)
#     # file_path = os.path.join(dir_path, f'{A}_시설정보.xlsx')
#     # df_merge_result_2018.to_excel(file_path, index=True)
#     return df_merge_result_2018
#
# df_merge_result=df_year(2022)
...
variables_to_log_transform = ['승강기수', '의사수', '병상수', '용적률산정연면적', '대지면적', '연면적', '지하층수', '지상층수', '층수', 'USE_QTY_kWh']
# df_new = df_new[(df_new[variables_to_log_transform] > 0).all(axis=1)]
print('# of data : ', df_new.shape[0])
print(' ')
# 로그 변환 적용
# df_new[variables_to_log_transform] = df_new[variables_to_log_transform].applymap(lambda x: np.log(x) if x > 0 else np.nan)
# df_new[variables_to_log_transform] = df_new[variables_to_log_transform].applymap(lambda x: np.sqrt(x) if x >= 0 else np.nan)
df_new[variables_to_log_transform] = df_new[variables_to_log_transform].applymap(lambda x: x**2)
# df_new[variables_to_log_transform] = df_new[variables_to_log_transform].applymap(lambda x: 1/x if x != 0 else np.nan)

#
# plt.figure(figsize=(10, 8))
# df_new.boxplot()
# # sns.violinplot(x='Columns', y='Values', data=df_melted)
# plt.xticks(rotation=90)  # x축 레이블이 겹치지 않도록 회전합니다.
# plt.tight_layout()
# plt.show()
#
#
#
# scaler=MinMaxScaler()
# df_new1 = scaler.fit_transform(df_new)
